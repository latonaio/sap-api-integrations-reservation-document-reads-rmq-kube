...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-reservation-document-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(reservation string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "ReservationDocumentHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "ReservationDocumentItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) Item(reservation, recordType, product string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "ReservationDocumentItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
